...
class AdaBelief(OptimizerDef):
  ...
  def apply_param_gradient(self, step, hyper_params, param, state, grad):
    ...
    grad_sq_ema = beta2 * state.grad_sq_ema + (1. - beta2) * lax.square(grad - grad_ema) + hyper_params.eps

    # bias correction
    t = step + 1.
    grad_ema_corr = grad_ema / (1 - beta1 ** t)
    grad_sq_ema_corr = (grad_sq_ema + hyper_params.eps) / (1 - beta2 ** t)

    denom = jnp.sqrt(grad_sq_ema_corr)

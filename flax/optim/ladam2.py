...
@struct.dataclass
class _Ladam2ParamState:
  grad_ema: onp.ndarray
  grad_mad_ema: onp.ndarray

class Ladam2(OptimizerDef):
  ...
  def __init__(self,
               ...
               eps=1e-8,
               weight_decay=0.0):
    """Constructor for the Ladam2 optimizer.
    Args:
    ...
    """
    hyper_params = _Ladam2HyperParams(learning_rate, beta1, beta2, eps, weight_decay)
    super().__init__(hyper_params)

  def init_param_state(self, param):
    return _Ladam2ParamState(jnp.zeros_like(param), jnp.zeros_like(param))
  ...
  def apply_param_gradient(self, step, hyper_params, param, state, grad):
    ...
    denom = jnp.sqrt(jnp.square(grad_ema_corr) + jnp.square(grad_mad_ema_corr)) + hyper_params.eps
    new_param = param - hyper_params.learning_rate * grad_ema_corr / denom
    new_param -= hyper_params.learning_rate * weight_decay * param
    new_state = _Ladam2ParamState(grad_ema, grad_mad_ema)
